...
21g-S2: COMPLETE (branch cycle21g/edit — plan fc09d1f+4590c9f, kernel-seam decision b30347d, Task 1 4497102 + review fix a1b827c)
  KERNEL SEAM SETTLED (spec 3.6a added): SelectionRange is visible-order indices; visible order is worker-owned; kernel's own fill/clipboard use PRIVATE workerCoord.getRowByIndex (commitFill cgrid.ts:2387, serializeRangesMainSide :4424); NO public surface -> genuine intrinsic need per no-retroactive-layering -> kernel gets public getRowsByIndex(rowIndexes): Promise<Array<{rowIndex,rowId,data}|null>> as Task 10 (own review gate; cycle's ONLY kernel diff). Target collection becomes async.
  Plan COMPLETE (Tasks 1-12, ~830 lines): A=1 scaffold/models/settings; B=2-4 journal/cascade+patches/preview; C=5-9 numericOps+smartEdit/magnitude/bulkUpdate/plusMinus/shortcuts; D/E=10 kernel seam, 11 bridge, 12 showcase+E2E+README+gates. Drafter code-verify corrections synced to spec+recon: destroyed-guard = all-null aligned array; isCellEditable + isEditing NOT public (recon C.9 correction; bridge replicates addon-side / derives editing flag from cellEditingStarted/Stopped); focused rowIndex synthesized from focus-collapsed 1x1 range (selectionModel.ts:130-151).
  Task 1: complete (4497102 + fix a1b827c, review APPROVED-after-fix). types.ts spec-verbatim models; settings.ts defaults/merge/recordSourceKey/shouldRecord; tsconfig rootDir scaffold bug fixed ("src"->"."); edit 13/13, tsc clean, root typecheck 21/21, kernel-diff gate EMPTY. Review found [Critical] mergeEditSettings aliased default enabledOps array (singleton corruption on result mutation) -> copy-on-fallback + deep no-aliasing regression test.
...
21g Task 10: complete (bccec6f, review at closeout — the cycle's ONLY kernel diff)
  Kernel public getRowsByIndex(rowIndexes) per spec 3.6a: Set-dedupe + Promise.all over workerCoord.getRowByIndex, 1:1 input alignment, out-of-range -> null, entry + post-Promise.all destroyed guards -> all-null aligned, empty input -> [] no worker traffic; CGrid method + api.ts doc + makeApi entry (cast-tripwire test case 7); harness mirrored from distinctValuesLimit.test.ts; kernel 2575/2575 (2568+7); scope proof: ONLY api.ts/cgrid.ts/getRowsByIndex.test.ts. PR #98 re-checked at S5 start: still OPEN (proceeded; shared-cgrid.ts merge to be done deliberately).
21g Task 11: complete (8b2e821, review at closeout)
  wireEditIntoKernel bridge: structural surface (isEditing/isCellEditable NOT public -> editing flag from cellEditingStarted/Stopped, editability replicated addon-side from columnDefs leaf-walk); focused rowIndex synthesized from focus-collapsed 1x1 range; row mirror (forEachRow seed / rowsChanged freshen / modelUpdated clear+reseed w/ 21f staleness regression); cellKeyDown router (+/=/- nudges, letter-set shortcuts, preventDefault ONLY on match, negative asserted); applyDirection writer = journal applier (dedupe -> clone -> valueParser->valueSetter replication w/ editor-path parity fixture, setter bool ignored -> ONE Tx; replaying guard); selection snapshot/restore around every programmatic apply; feeds (cell-editor from cellValueChanged w/ colMeta field + Object.is skip; stream from rowsChanged transaction* gated default-off); facades (enforceSingleColumn -> applied:0 no Tx; confirmThreshold never blocks; distinctValues via makeDistinctValuesFeed); idempotency + destroy + gridPreDestroyed. 30 tests. Deviations logged in report (adapter coverage split across call sites; label conventions).
21g-S5: COMPLETE. edit 157/157; kernel 2575/2575; tsc clean; root typecheck 21/21; kernel diff = Task 10's three files ONLY; expression/format/rules/calc EMPTY. NEXT UNIT: 21g-S6 (Task 12 showcase+E2E+README; then SINGLE CLOSEOUT REVIEW of Tasks 2-12 + fix wave; PR + merge. Re-check PR #98 first — hard dep for Task 12 tripwire + icons).
21g Task 12: complete (dd9a694)
  edit-blotter showcase (12 rows, smart-edit toolbar, bulk-update + distinct select, undo/redo + live count, 2 seeded nudges incl status=="active" gate, 2 shortcuts, magnitude-wrapped price, window.__cgridEdit) + 8-behavior E2E + README rewrite. DISCOVERY (live smoke): kernel rowIdAt() stub feeds synthetic ids into cellKeyDown -> nudges/shortcuts DEAD against real kernel until PR #98 (same root cause as 21f F5) -> E2E tests 4/5/8 authored as test.fail tripwires (flip to unexpected-pass at #98), tests 1/2/3/6/7 pass real. Gate ledger: edit 157/157, kernel 2575/2575, typecheck 21/21, lint clean, showcase E2E 158 pass + 6 visual, kernel diff = Task 10 files only, NUL/straggler clean.
21g CLOSEOUT REVIEW (fable, whole branch, per user single-review directive): NEEDS-FIXES -> fix wave 9a9d66b -> re-review READY TO MERGE
  [Important] applyDirection re-parsed values on undo/redo replay (non-idempotent parser corrupts undo) -> mode 'initial'|'replay', parser only on initial; percent-parser regression tests. [Important] key router skipped editability -> resolveEditable in buildFocusedTarget, non-editable = no-match (no preventDefault). [Minor taken] journal.record defensive patch-array copy; applied-count = actually-written rows. Carried minors: preventDefault-before-empty-patch (spec-consistent); programmatic redo replays pre-parse computed value (undo exact everywhere; full fix = pinned-semantics change, carry to 21h/21i); applied==0 still records entry (tick-race territory); patch OBJECTS shared by reference in journal copy.
21g-S6: gates all green at 9a9d66b (edit 161/161, kernel 2575/2575, typecheck 21/21, lint clean, build 13/13, E2E 158+6). PROCEEDING to PR + squash-merge.
21g-S6: COMPLETE — 21g squash-merged as PR #99 @ b737741 (2026-07-03); main ff-only synced; baselines updated (kernel 2575, edit 161, showcase E2E 158+6 incl 4 tripwires). CYCLE 21g DONE.
