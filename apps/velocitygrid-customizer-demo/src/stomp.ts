...
export interface Position {
  ...
  // grouping/pivot affordances have meaningful categorical columns.
  desk?: string;
  region?: string;
  currency?: string;
  trader?: string;
 ...
 *  keeps E2E deterministic; `?stress=heavy` matches velocitygrid-positions. */
const KNOB_SETS = {
...
/**
 * Protocol (same stomp-view-server as velocitygrid-positions / showcase):
 * subscribe `/snapshot/positions/<clientId>` — snapshot batches then live
 * ticks on one channel; publish a trigger to
