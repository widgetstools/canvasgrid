...
// one `<input class="vg-floating-filter-input" data-vg-col-id>` per column
...
const ffVisibleCount = (page: Page) => page.evaluate(() =>
  Array.from(document.querySelectorAll('.vg-floating-filter-input'))
...
test('popover flags: floating filter + set filter + groupable write templates; persists across reload', async ({ page }) => {
  ...
  await page.waitForFunction((n) => Array.from(document.querySelectorAll('.vg-floating-filter-input'))
...
test('filter type Set opens the real set-filter popup from the header', async ({ page }) => {
  ...
  await page.keyboard.press('Escape');
  await expect(page.locator('.vgext-menu.vgext-col')).toHaveCount(0);
  ...
  await page.locator('button[data-vg-floating-filter-expand][data-vg-col-id="dailyPnl"]').click();
  await expect(page.locator('.vg-filter-popup-set')).toBeVisible();
});

test('groupable off rejects the column at the row-group panel; groupable on accepts it', async ({ page }) => {
  await selectCol(page, 'pnl');
  await openPanel(page);
  await row(page, 'enableRowGroup').locator('.vgext-col-switch').click(); // default false -> true
  ...
  await openPanel(page);
  await row(page, 'enableRowGroup').locator('.vgext-col-switch').click(); // true -> false
...
// `pnl` carries a STATIC `aggFunc: 'sum'` in its colDef (apps/velocitygrid-ext-demo/
...
// feeds back into `ResolvedColDef.aggFunc` (no call site in velocityGrid.ts patches
...
test('Show-in-header off hides the sum(...) header caption; back on restores it', async ({ page }) => {
  ...
  await openPanel(page);
  await row(page, 'aggHeader').locator('.vgext-col-switch').click(); // Show-in-header off
  ...
  await openPanel(page);
  await row(page, 'aggHeader').locator('.vgext-col-switch').click(); // Show-in-header back on
