...
window.addEventListener('unhandledrejection', (event) => {
  console.error('[velocitygrid-ext-demo] unhandled rejection:', event.reason);
});
window.addEventListener('error', (event) => {
  console.error('[velocitygrid-ext-demo] error event:', event.error || event.message);
});
...
try {
      ...
      extensions: [
        ...
        ...titleBarExtensions({
          ...
          onDateChange: (iso) => {
            ...
            console.info('[velocitygrid-ext-demo] as-of date →', iso);
          },
        }),
  ...
  console.error('[velocitygrid-ext-demo] VelocityGridExt construction failed', err);
  throw err;
}
...
ext.reapplyActiveProfile().catch((err) => {
  console.error('[velocitygrid-ext-demo] failed to reapply the active profile', err);
});

// Expose for console poking + the hermetic E2E suite.
(window as unknown as { __ext: unknown }).__ext = ext;
console.log('[velocitygrid-ext-demo] __ext initialized, grid ready:', !!ext.grid);
...
if (PAINT_HARNESS) {
  ...
  connectStomp({
    onPhase: (phase) => {
      console.info(`[velocitygrid-ext-demo] stomp phase: ${phase}`);
    ...
    onSnapshot: (rows) => {
      ...
      console.info(
        `[velocitygrid-ext-demo] discovered ${defs.length} column roots / ` +
