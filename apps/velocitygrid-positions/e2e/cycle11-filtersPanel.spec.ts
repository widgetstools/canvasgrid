...
test.describe('Cycle 11 / Task 4 — Filters tool panel', () => {
  test('opens with the canonical layout — search input, expand-all button, collapsible column rows', async ({ page }) => {
    ...
    await expect(page.locator(`${PANEL} .vg-filters-panel-search input`)).toBeVisible();
    await expect(page.locator(`${PANEL} .vg-filters-panel-expand-all button`)).toBeVisible();

    // Demo grid carries 17 columns, most filterable. Sanity-check the
    // first few row labels are the column headerNames (via
    // getColumnHeaderName).
    const labels = page.locator(`${PANEL} .vg-filters-panel-row-label`);
    ...
    const expanded = page.locator(`${PANEL} .vg-filters-panel-row[data-expanded="true"]`);
    await expect(expanded).toHaveCount(0);
    // The chevron renders as the `›` glyph.
    const firstChevron = page.locator(`${PANEL} .vg-filters-panel-row .vg-filters-panel-row-chevron`).first();
  ...
  test('clicking a row expands it and mounts the column filter editor inline', async ({ page }) => {
    ...
    // hallmark.
    const priceRow = page.locator(`${PANEL} .vg-filters-panel-row[data-col-id="currentPrice"]`);
    await priceRow.locator('.vg-filters-panel-row-header').click();
    await waitForFrames(page, 3);

    await expect(priceRow).toHaveAttribute('data-expanded', 'true');
    await expect(priceRow.locator('.vg-filters-panel-row-chevron')).toHaveText('⌄'); // ⌄
    // The reused filter-popup body shows up under the row's editor
    // host. Specifically the `vg-filter-popup-number` class signals
    // the number-filter editor mounted (not a stub).
    await expect(priceRow.locator('.vg-filter-popup-number')).toBeVisible();
    await expect(priceRow.locator('select.vg-filter-popup-operator')).toBeVisible();
  });
  ...
  test('committing a filter through the inline editor propagates to the grid filter model', async ({ page }) => {
    ...
    // wiring.
    await priceRow.locator('select.vg-filter-popup-operator').selectOption('greaterThan');
    await priceRow.locator('input.vg-filter-popup-input[data-vg-filter-input="primary"]').fill('150');
    // Apply commits the model via setColumnFilterModel.
    await priceRow.locator('button.vg-filter-popup-button-apply').click();
    await waitForFrames(page, 6);
  ...
  test('only one row stays expanded when the user clicks a second row (the first auto-collapses)', async ({ page }) => {
    ...
    const priceRow = page.locator(`${PANEL} .vg-filters-panel-row[data-col-id="currentPrice"]`);
    const yieldRow = page.locator(`${PANEL} .vg-filters-panel-row[data-col-id="yield"]`);

    await priceRow.locator('.vg-filters-panel-row-header').click();
    await waitForFrames(page, 3);
    await expect(priceRow).toHaveAttribute('data-expanded', 'true');

    await yieldRow.locator('.vg-filters-panel-row-header').click();
    await waitForFrames(page, 3);
  ...
  test('clicking an expanded row collapses it back to the chevron-only state', async ({ page }) => {
    ...
    await waitForFrames(page, 3);
    await expect(priceRow).toHaveAttribute('data-expanded', 'false');
    await expect(priceRow.locator('.vg-filter-popup-number')).toHaveCount(0);
  });
  ...
  test('search input filters the row list (case-insensitive substring on header + colId)', async ({ page }) => {
    ...
    const search = page.locator(`${PANEL} .vg-filters-panel-search input`);
    await search.fill('pri');
    await waitForFrames(page, 3);
    const visibleRows = page.locator(`${PANEL} .vg-filters-panel-row:not([style*="display: none"])`);
    ...
    await search.fill('');
    await waitForFrames(page, 3);
    const allRows = await page.locator(`${PANEL} .vg-filters-panel-row`).count();
    const restored = await page.locator(`${PANEL} .vg-filters-panel-row:not([style*="display: none"])`).count();
  ...
  test('expand-all button expands every row in a single click; clicking again collapses them', async ({ page }) => {
    ...
    const expandBtn = page.locator(`${PANEL} .vg-filters-panel-expand-all button`);
    const rows = page.locator(`${PANEL} .vg-filters-panel-row`);
    ...
    await expandBtn.click();
    await waitForFrames(page, 3);
    const stillExpanded = page.locator(`${PANEL} .vg-filters-panel-row[data-expanded="true"]`);
