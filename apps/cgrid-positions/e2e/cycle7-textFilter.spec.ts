...
test.describe('Cycle 7 / Task 5 — text-filter popup', () => {
  ...
  test('operator select carries the eight text-filter options', async ({ page }) => {
    await gridReady(page);
    await page.locator('button[data-vg-floating-filter-expand][data-vg-col-id="cusip"]').click();
    const select = page.locator('.vg-filter-popup-text select');
  ...
  test('caseSensitive checkbox renders by default', async ({ page }) => {
    await gridReady(page);
    await page.locator('button[data-vg-floating-filter-expand][data-vg-col-id="cusip"]').click();
    const cb = page.locator('.vg-filter-popup-text input[data-vg-filter-case-sensitive]');
  ...
  test('startsWith + literal-no-match + Apply reduces the displayed row count', async ({ page }) => {
    ...
    await popup.locator('input[data-vg-filter-input="primary"]').fill('ZZZZZZ');
  ...
  test('Reset returns the row count to the original', async ({ page }) => {
    ...
    await page.locator('button[data-vg-floating-filter-expand][data-vg-col-id="cusip"]').click();
    await page.locator('.vg-filter-popup-text button[data-vg-filter-action="reset"]').click();
    await waitForFrames(page);
  ...
  test('VelocityGridApi.showColumnFilter / hideColumnFilter round-trip', async ({ page }) => {
    await gridReady(page);
    await page.evaluate(
      () => (window as unknown as { __velocity-grid: GridApiSurface }).__cgrid.showColumnFilter('cusip'),
    );
    await expect(page.locator('.vg-filter-popup-text')).toHaveCount(1);
    await page.evaluate(
      () => (window as unknown as { __velocity-grid: GridApiSurface }).__cgrid.hideColumnFilter(),
    );
    await expect(page.locator('.vg-filter-popup-text')).toHaveCount(0);
  });
});
