 ...
 *   6. `VelocityGridApi.showColumnFilter` / `hideColumnFilter` round-trip
...
test.describe('Cycle 7 / Task 9 — set-filter popup', () => {
  test('clicking the expand button opens the set-filter popup with one checkbox per distinct ticker value', async ({ page }) => {
    ...
    const expand = page.locator('button[data-vg-floating-filter-expand][data-vg-col-id="ticker"]');
    await expect(expand).toHaveCount(1);
    await expand.click();
    const popup = page.locator('.vg-filter-popup-set');
    ...
    const checkboxes = popup.locator('input[data-vg-set-filter-value]');
  ...
  test('unchecking one value + Apply reduces the displayed row count', async ({ page }) => {
    ...
    await page.locator('button[data-vg-floating-filter-expand][data-vg-col-id="ticker"]').click();
    const popup = page.locator('.vg-filter-popup-set');
    const first = popup.locator('input[data-vg-set-filter-value]').first();
    await first.uncheck();
  ...
  test('Reset restores the row count to the original', async ({ page }) => {
    ...
    await page.locator('button[data-vg-floating-filter-expand][data-vg-col-id="ticker"]').click();
    const popup = page.locator('.vg-filter-popup-set');
    await popup.locator('input[data-vg-set-filter-value]').first().uncheck();
    await popup.locator('button[data-vg-filter-action="apply"]').click();
    await expect.poll(
      async () => page.evaluate(
        () => (window as unknown as { __velocity-grid: GridApiSurface }).__cgrid.getDisplayedRowCount(),
    ...
    await page.locator('button[data-vg-floating-filter-expand][data-vg-col-id="ticker"]').click();
    await page.locator('.vg-filter-popup-set button[data-vg-filter-action="reset"]').click();
  ...
  test('VelocityGridApi.showColumnFilter / hideColumnFilter round-trip drives the set popup', async ({ page }) => {
    await gridReady(page);
    await page.evaluate(
      () => (window as unknown as { __velocity-grid: GridApiSurface }).__cgrid.showColumnFilter('ticker'),
    );
    await expect(page.locator('.vg-filter-popup-set')).toHaveCount(1);
    await page.evaluate(
      () => (window as unknown as { __velocity-grid: GridApiSurface }).__cgrid.hideColumnFilter(),
    );
    await expect(page.locator('.vg-filter-popup-set')).toHaveCount(0);
  });

  test('per-column API — setColumnFilterModel + isColumnFilterPresent + destroyFilter round-trip', async ({ page }) => {
    await gridReady(page);
    const initial = await page.evaluate(
      () => (window as unknown as { __velocity-grid: GridApiSurface }).__cgrid.isColumnFilterPresent(),
    );
    ...
    await page.evaluate(
      () => (window as unknown as { __velocity-grid: GridApiSurface }).__cgrid.destroyFilter('ticker'),
    );
    await waitForFrames(page);
