...
test.describe('Cycle 7 / Task 3 — number-filter popup', () => {
  test('clicking the expand button opens the popup', async ({ page }) => {
    ...
    const expand = page.locator('button[data-cg-floating-filter-expand][data-cg-col-id="notionalAmount"]');
  ...
  test('greaterThan + value + Apply reduces the displayed row count', async ({ page }) => {
    ...
    await page.locator('button[data-cg-floating-filter-expand][data-cg-col-id="notionalAmount"]').click();
    const popup = page.locator('.cg-filter-popup-number');
    await popup.locator('select').selectOption('greaterThan');
    await popup.locator('input[data-cg-filter-input="primary"]').fill('1000000');
  ...
  test('inRange shows two inputs and applies a range filter', async ({ page }) => {
    ...
    const visiblePrimary = popup.locator('input[data-cg-filter-input="primary"]');
    const visibleSecondary = popup.locator('input[data-cg-filter-input="secondary"]');
  ...
  test('CGridApi.showColumnFilter / hideColumnFilter round-trip', async ({ page }) => {
    ...
    await expect(page.locator('.cg-filter-popup-number')).toHaveCount(1);
    await page.evaluate(
      () => (window as unknown as { __cgrid: GridApiSurface }).__cgrid.hideColumnFilter(),
    );
    await expect(page.locator('.cg-filter-popup-number')).toHaveCount(0);
  });
});
