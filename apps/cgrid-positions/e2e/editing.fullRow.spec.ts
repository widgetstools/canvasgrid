...
test.describe('Cell editing — full-row mode (Cycle 5 / Task 10)', () => {
  ...
  test('clicking cusip opens editors for all editable cells in the row + rowEditingStarted fires', async ({ page }) => {
    ...
    await expect(page.locator('.vg-editor-overlay--row input.vg-cell-editor--text').first()).toBeVisible();
    await expect(page.locator('.vg-editor-overlay--row select.vg-cell-editor--select')).toBeVisible();
    await expect(page.locator('.vg-editor-overlay--row input.vg-cell-editor--number')).toBeVisible();
    // Count is ≥ 3 — exact count depends on the visible-column window but
    // cusip, ticker, notional are reliably in view at 800×600.
    const editorCount = await page.locator('.vg-editor-overlay--row').count();
  ...
  test('Tab cycles focus through editors within the row', async ({ page }) => {
    await clickCell(page, 0, 'cusip');
    const cusipInput = page.locator('.vg-editor-overlay--row input.vg-cell-editor--text').first();
    ...
    await expect(page.locator('.vg-editor-overlay--row select.vg-cell-editor--select')).toBeFocused();
  });
  ...
  test('Enter commits every changed editor + rowEditingStopped + rowValueChanged fire', async ({ page }) => {
    ...
    await expect(page.locator('.vg-editor-overlay--row')).toHaveCount(0);
