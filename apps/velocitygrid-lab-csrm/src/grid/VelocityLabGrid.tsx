...
import { wireLabFeatures, type LabWiring, type WireLabOptions } from './wiring';
...
export interface VelocityLabGridProps {
  ...
  /** Pre-authored feature configuration for this tab — see `lab/seeds.ts`. */
  wiring: WireLabOptions;
...
export function VelocityLabGrid({ gridId, title, options, wiring: wiringOpts, onReady, onTeardown }: VelocityLabGridProps) {
  ...
  const latest = useRef({ options, onReady, onTeardown, title, wiringOpts });
  latest.current = { options, onReady, onTeardown, title, wiringOpts };

  useEffect(() => {
    const host = hostRef.current;
    if (!host) return;

    const { options: opts, onReady: ready, title: name, wiringOpts: wOpts } = latest.current;
    ...
    wiring = wireLabFeatures(ext.grid, wOpts);
