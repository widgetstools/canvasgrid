...
// pills come from titleBarExtensions() → savedFiltersItem()`,
  ...
  // ── Data ──────────────────────────────────────────────────────────
  {
    ...
    title: 'Live updates',
    subtitle: 'A high-frequency stream into the client-side row model. Every tick is a transaction, and only the cells that changed repaint.',
    columns: () => pickColumns([
      'cusip', 'ticker', 'instrumentDescription', 'compositeRating',
    ...
    guide: {
      what: 'Ten thousand rows, a quarter-second cadence, and a repaint that touches only the damaged region. The flash mask comes from the worker with the chunk, so the highlight marks what actually changed rather than what happened to be on screen.',
      try: [
...
// snapshot → grid.setRowData(rows)
// ticks    → grid.applyTransactionAsync({ update: changed })`,
    },
  },
  {
    id: 'grouping',
    ...
    guide: {
      what: 'Aggregation runs in the worker over the whole book, not over the visible rows, so a collapsed group shows the true sum of everything under it. Totals recompute as ticks land, which is why the pinned grand total moves while you watch it.',
      try: [
      ...
      config: `{ field: 'desk',   rowGroup: true, hide: true }
...
{ when: 'dailyPnL < -250000',
  ...
  // ── Editing ───────────────────────────────────────────────────────
  {
    ...
    guide: {
      what: 'The interesting case in a trading blotter is not editing — it is editing while the row underneath you is moving. Edits and ticks are both transactions against one book, so the last write wins and the grid never shows a half-applied row.',
      try: [
