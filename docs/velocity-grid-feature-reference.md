...
| Hub identity | `(origin, script URL, name)`. `connectHub({ workerUrl, name, strict })`; `name` is in practice the **app name** and rides in the URL as `?app=` (the worker options must stay a static literal — Vite's serve/test transform throws on a variable there, though `vite build` tolerates it; see §5.7's bundling row). `getDataHubTarget()` → `{ url, name, bundled }` |
...
| Feed in the worker — **the default** (`workerFeed: false` / `?feed=main` to opt out) | The STOMP transport runs **inside the SharedWorker**, so there is one feed because there is one worker — no lock, no takeover gap, and no main-thread → worker hop per update. `feed:start` is start-or-**join**, keyed on table name. Needs a shared engine and a deployed worker at protocol ≥ 2 (`canUseWorkerFeed()`); anything else falls back to the main-thread feed silently and by design. `BookTelemetry.feedRole` = `worker` when it took |
| Config disagreement | Table identity folds in `providerId` but **not** the resolved config, so two apps resolving one provider to different topics land on one feed and the first caller's config decides. Joining still beats two feeds writing one table, so this reports rather than resolves: `WorkerFeedState.configMismatch` carries the differing fields to every subscriber and the book warns once. Compared on *effective* values — a per-page random `clientId` is not a disagreement, the topic it resolves to is |
...
| Reconnect | `onConnected` fires on RECONNECT too and re-requests the snapshot, so it must clear `snapshotComplete` — otherwise the end token is ignored and the book sits in `snapshot` forever while rows arrive. Applies to **both** paths; `book.ts` had the bug first. Pinned by `npm run verify:worker-feed-reconnect`, which severs a relay in front of the broker |
| Rate staleness | `liveUpdatesPerSec` is a one-second window and worker state is pushed when rows arrive, so a feed that goes quiet would report its last rate forever. The worker sends one trailing state ~1.1s after the window empties; a running feed reschedules it and never pays for it |
...
| Target model | **`(origin, instance name)` with `bundled: false`** — an app joins the engine *named* `name` on its origin. Reached by fixing the URL to a deployed constant, which leaves the name as the only varying axis. The name rides **in that URL** as `?engine=`, not in the `SharedWorker` options: Vite `eval`s the options object, so a variable there is either a build failure or a silently skipped worker transform depending on version. `name` without `url` warns and partitions nothing |
...
| `strict` | Turns both silent fallbacks — bundled copy, and dedicated-worker-on-failure — into a throw at `getPerspectiveClient()`, before the WASM fetch. Silence is the wrong default across several apps: everything works, just with N engines, N tables and N feeds |
| `name` | Leave at the default to put every blotter on the origin's one engine (one engine hosts many providers' tables, keyed by `providerId` + schema). Set it to deliberately partition |
| Cost of *not* converging | Two engines, two copies of the table, two feeds. It used to be worse than that: feed leadership is a Web Lock, which is **origin**-scoped while the engine is not, so two apps contended for one lock over two separately-empty tables and the loser waited out a 30s `waitForSharedSnapshot` timeout. That is a property of the MAIN-THREAD feed, and the default no longer has it — same two apps, measured: `?feed=main` 34.9s to live, default 3.5s. Deploying one worker is still what removes the duplication |
| Bundling hazard | Two separate rules, with **different** consequences. (1) `new URL('./sharedServer.worker.ts', import.meta.url)` must stay **literal and inline** inside `new SharedWorker(...)` — computing it elsewhere degrades the build to a bare `.ts` asset that fails only in production. (2) The options object must stay a static literal (`{ type: 'module' }`) — measured on Vite 7.3.6, `vite build` *tolerates* a variable here and still compiles the worker correctly, but the serve/test transform throws: `{ name, type: 'module' }` made **11 `packages/ext` suites unloadable**, hiding 50 tests. So (1) is a production hazard and (2) is a test hazard; both are pinned by `packages/perspective/tests/sharedWorkerBundling.test.ts` |
