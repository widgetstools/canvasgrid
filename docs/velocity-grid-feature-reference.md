...
| Bundling hazard | The default path depends on `new URL('./sharedServer.worker.ts', import.meta.url)` staying **literal and inline** inside `new SharedWorker(...)`, AND on the options object staying a static literal (`{ type: 'module' }`). Either one broken degrades the build to a bare `.ts` asset that fails only in production — or, under vitest's stricter Vite, throws at import: `{ name, type: 'module' }` here made **11 `packages/ext` suites unloadable**, hiding 50 tests, before it was traced. Both halves pinned by `packages/perspective/tests/sharedWorkerBundling.test.ts` |
