...
`VelocityGrid<TRow>` in [velocityGrid.ts](cgrid/src/velocityGrid.ts) is a **god object**:
...
1. Identify the seam: the set of fields + methods on `VelocityGrid` that
...
3. `VelocityGrid` holds the manager as a field; delegating wrappers stay on
   `VelocityGrid` for backward compatibility with internal callers.
...
- **God object**: [velocityGrid.ts](cgrid/src/velocityGrid.ts) lines 660-700 hold ~70
...
- **Type kitchen sink**: [types.ts](cgrid/src/types.ts) — 3,170 lines,
  ~80 flat exports, zero domain grouping. `VelocityGridOptions` alone is ~960
  lines (293–1253).
- **Panel multiplexer**:
...
| 2 | **Extract `ViewportManager`** — scroll state (`scrollLeft`, `scrollTop`, `scrollVelocityRows`, `scrollEndTimer`, `lastScrollSample*`), `recomputeViewport`, `requestViewport`, prefetch range, viewport invalidation. Owns the scroller listener (already routed through `disposables`). `VelocityGrid` delegates via `this.viewport.recompute()` / `this.viewport.request()`. | `cgrid/src/core/viewportManager.ts` (new), `velocityGrid.ts` (slim) | `viewportManager.test.ts` (new) + integration green | Med |
| 3 | **Extract `WorkerCoordinator`** — `workerClient` ownership, transaction queue, async-tx flushing, viewport request dispatch, the main↔worker pipeline glue (`handleViewport`, `requestViewport` plumbing). `VelocityGrid` becomes the consumer. | `cgrid/src/core/workerCoordinator.ts` (new), `velocityGrid.ts` | `workerCoordinator.test.ts` + integration green | Med |
| 4 | **Extract `EditController`** — `editor`, `rowEdit`, `activeEdit`, the capture-phase keydown matrix (already routed through `disposables`), the `editorContainer` mousedown handler, `nextEditableCell`, `openEditor` / `stopEditing` lifecycle, full-row-edit bridging. | `cgrid/src/core/editController.ts` (new), `velocityGrid.ts` | `editController.test.ts` + integration green | Med |
| 5 | **Extract `PivotEngine` + `GroupingCoordinator` + `ColumnStateManager`** — three sibling state extractions (the existing `PivotState` / `GroupingState` are just the data models; the coordinators own the cgrid-side wiring: column tree swaps, scrollLeft preservation across pivot toggles, subscribe/unsubscribe loops, default expansion materialisation, `setRowGroupColumns`/`setPivotColumns`/`setValueColumns` API). One PR per coordinator. **PivotEngine split into 5a + 5b** — 5a (**LANDED**, commit `refactor/cycle19-task-5a-pivot-engine`) is pure extraction: `PivotEngine` owns PivotState + pivotActive + primaryColumnTree + cellSpecById + treeSignature + the state-change handler + the tree-swap orchestration + `maybeSyncPivotColumns` + the runtime option flips; `VelocityGrid` delegates every pivot API method + calls `pivotEngine.maybeSyncPivotColumns(chunk)` at chunk-arrival. velocityGrid.ts drops 208 LoC (8,077 → 7,869); pivotEngine.ts + tests add 506 + ~370. Zero behaviour change; 2,264 unit tests green (17 new); positions 229 pass / 2 fixme, showcase 75 pass. **5b (follow-up PR — DEFERRED)** carries the behavioural gap the master analysis called out: when `setPivotMode(true)` flips on, auto-hide primaries (AG-v36 parity), flip `computeRowChecked` in `columnsPanel.ts` from `visible OR role` to `role only`, un-fixme the two `cycle18-task5-pivotToolPanel.spec.ts:221, 252` specs. This required a semantic decision on the `panelDragRouting.spec.ts:87, 351` assertions (they hard-encode the lenient semantic + the "hide flag is sole authority" invariant from commit `89c7a74`); rewriting them coherently belongs with the auto-hide, not bundled into the extraction PR. | `cgrid/src/core/pivotEngine.ts`, `cgrid/src/core/groupingCoordinator.ts`, `cgrid/src/core/columnStateManager.ts` (all new), `velocityGrid.ts`, `cgrid/src/interaction/toolPanels/columnsPanel.ts` | `pivotEngine.test.ts` (17 tests, ✓), `groupingCoordinator.test.ts`, `columnStateManager.test.ts` + integration green; re-enable the two fixme E2E specs in 5b | Med-High |
...
- Public API surface is frozen for cycle 19. Anything that changes the `VelocityGridApi` shape is out of scope until cycle 20.
...
2. `VelocityGrid` (or `worker.ts` / `columnsPanel.ts`) reduced by the
