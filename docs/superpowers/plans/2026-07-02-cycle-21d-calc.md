# Cycle 21d — `@wellsfargo-starui/velocity-grid-calc` (Calculated Columns + Delta Aggregates + Overrides/Templates) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship `@wellsfargo-starui/velocity-grid-calc` — worker-evaluated calculated columns (row-local + scoped aggregates + `PREV`), the delta-aware aggregate registry + cache, the column override/template fold — with the kernel CalcPass and bridge, per the [design spec](../specs/2026-07-02-cycle-21d-calc-design.md).

**Architecture:** Thread-split engine: `@wellsfargo-starui/velocity-grid-calc` compiles (post-parse aggregate transform on `@wellsfargo-starui/velocity-grid-expression`'s portable AST → two-pass split) and serializes a self-contained AST interpreter + delta-aggregate impls to the kernel worker (the `aggFuncRegistry` `Function.toString()` precedent). Kernel gains a calc DI slot, a two-stage `CalcPass` (Stage A row-local before filter/sort; Stage B aggregate-dependent after grouping, cache keyed `(fn,colId,scopeKey,dataVersion)`), and colDef synthesis/override folding. `packages/expression` untouched.
...
- **Dep graph:** calc → expression + format (deps), kernel peerDep touched only via `wireIntoKernel`'s structural surface; kernel gets `@wellsfargo-starui/velocity-grid-calc` devDep (types in tests) with ZERO runtime imports (grep-verified at gates).
...
**`@wellsfargo-starui/velocity-grid-calc` — all new:** `src/types.ts`, `src/aggTransform.ts`, `src/compile.ts`, `src/aggregates/{contract,basic,stats,share,registry}.ts`, `src/scopeKey.ts`, `src/calcEngine.ts`, `src/templates.ts`, `src/overrides.ts`, `src/workerProgram.ts`, `src/bridge.ts`, `src/index.ts`, `vitest.config.ts`, `README.md`, `tests/**` mirroring src.

**`@wellsfargo-starui/velocity-grid` — new:** `src/core/calcSlot.ts`, `src/worker/passes/calcPass.ts`.
**`@wellsfargo-starui/velocity-grid` — touched:** `src/velocityGrid.ts` (registerCalcProvider; colDef rebuild consults slot; program shipping; getDistinctValues limit), `src/types/api.ts`, `src/worker/protocol.ts` (+`setCalcProgram` message, calc program payload types, distinct limit), `src/worker/dispatch.ts`, `src/worker/dataPipeline.ts` (CalcPass wiring: Stage A pre-filter, Stage B post-group), `src/worker/client.ts` (setCalcProgram + limit), `packages/kernel/package.json` (+`@wellsfargo-starui/velocity-grid-calc` devDep).
**Untouched (gate-verified):** `packages/expression/**`, `packages/rules/**`, `packages/format/**`, root configs, `eslint.config.mjs`.
**Showcase (Task 15):** `apps/cgrid-showcase/src/features/calculatedColumns.ts` (+ registration in `src/features/index.ts`, `@wellsfargo-starui/velocity-grid-calc` dep in `package.json`), `apps/cgrid-showcase/e2e/calculatedColumns.spec.ts`. Sources are TypeScript; never touch `.js` emit stragglers. E2E needs the dev server on :5185 (`npm run dev`; no playwright webServer block).
...
**Files:**
- Modify: `packages/calc/package.json` (kernel → peerDependencies; add `test:coverage` + `@vitest/coverage-v8` devDep — byte-mirror `packages/rules/package.json` but with deps `@wellsfargo-starui/velocity-grid-expression` + `@wellsfargo-starui/velocity-grid-format`)
...
**Interfaces:** Produces every public type + skeleton signature all later tasks import. Consumes `@wellsfargo-starui/velocity-grid-expression` types (Ast, Loc, Schema).
...
- **Calc owns its aggregate name lists:** `@wellsfargo-starui/velocity-grid-expression` does NOT export `AGGREGATE_NAMES` from its index (recon 2026-07-02), and calc's list is a superset anyway (statistical + share families). `aggTransform.ts` declares `CALC_AGGREGATE_NAMES` (shippable) and `ORDER_DEPENDENT_NAMES` (reserves) as its own consts.
...
- Consumes (from Task 1 / dependencies):
    ...
    - `@wellsfargo-starui/velocity-grid-expression`: `parse(source) → ParseResult` (tests only); types `Ast`, `AstNode`, `CallNode`, `FieldNode`, `AggregateNode`, `PrevNode`, `Loc`, `Schema`
...
- Consumes (from Task 2 / dependencies):
    ...
    - `@wellsfargo-starui/velocity-grid-expression`: `parse(source) → ParseResult`, `compile(ast) → CompileResult`, `evaluate(compiled, { row }) → unknown` (throws `EvalError`), `EvalError`; types `Ast`, `AstNode`, `Compiled`, `Schema`
...
- Consumes (from Tasks 2–3 / dependencies):
    ...
    - `@wellsfargo-starui/velocity-grid-expression` (tests only): `parse`, `compile`, `evaluate`, `EvalError` — the parity oracle
...
- **Built-ins skip the smoke test at module load** (`skipSmokeTest: true` on the internal path): keeps `import '@wellsfargo-starui/velocity-grid-calc'` eval-free on strict-CSP main threads; their round-trip is covered by the serialization tests instead. Public `registerAggregate` never skips.
...
**Interfaces:**
- Consumes:
    - Task 1 types (`Aggregate`, `AggScope`, `AggSpec`, `CalcValidationError`); Task 5's `registerFactory` / `getAggregate` / `serializeAggregates` + arity convention; `@wellsfargo-starui/velocity-grid-expression` `AstNode` shapes (AggregateNode `{kind:'aggregate', name, args, loc}` — no scope/slot fields of its own).
...
import type { AstNode } from '@wellsfargo-starui/velocity-grid-expression';
...
```bash
cd /Users/develop/wfh/canvasgrid && npx turbo run test --filter=@wellsfargo-starui/velocity-grid-expression --filter=@wellsfargo-starui/velocity-grid-format --filter=@wellsfargo-starui/velocity-grid-rules
```
...
- Consumes:
    ...
    - `@wellsfargo-starui/velocity-grid-format`: `compileFormat(source: FormatSource, opts?): CompileFormatResult` (`{ ok: true; program } | { ok: false; error: { kind: 'compile-format'; code; message; loc } }`) — validation only; the kernel re-compiles the string at ColDef-resolve
    - Task 1 types: `CalculatedColumnDef`, `CompiledCalc`, `CalcValidationError`, `Unsubscribe`, `CellDataType`; `Schema` from `@wellsfargo-starui/velocity-grid-expression`
...
- Consumes:
    ...
    - `@wellsfargo-starui/velocity-grid-format` `compileFormat` (override/template format-string validation)
...
> 4. **There is NO public `api.getDistinctValues`** despite spec §1.1 item 11 ("kernel already ships `api.getDistinctValues(colId)`"). What exists: the worker RPC (`protocol.ts:465`, handler `handlers/filter.ts:83-89`), `WorkerClient.getDistinctValues` (`client.ts:352-356`), `WorkerCoordinator.getDistinctValues` (`workerCoordinator.ts:251-253`), and ONE internal consumer — the set-filter popup (`velocityGrid.ts:2906-2908`). Task 13 therefore ADDS the public `VelocityGridApi.getDistinctValues(colId, limit?)` method (types/api.ts + velocityGrid.ts method + api-literal entry) in addition to threading `limit`.
> 5. **The client/coordinator method is `updateColumns`, not `setColumns`** (`client.ts:454-456`, call sites `velocityGrid.ts:1714` via init payload and `velocityGrid.ts:5071`).
> 6. **`columnDefs` is initial-only on `setGridOption`** (`velocityGrid.ts:5046-5051`); the runtime rebuild path is `updateGridOptions({ columnDefs })` → `rebuildColumns()` (`velocityGrid.ts:5063-5100`, `:5217-5235`). The calc fold hooks `resolveColumnTree`'s input at BOTH build sites (constructor `:941` + `rebuildColumns` `:5218`), and `registerCalcProvider` triggers the same rebuild + `workerCoord.updateColumns` sequence `updateGridOptions` uses.
...
- **Produces for Tasks 14–15:** `VelocityGrid.registerCalcProvider(provider)` + `VelocityGridApi.registerCalcProvider` (Task 9), `VelocityGridApi.getDistinctValues(colId, limit?)` (Task 13), kernel exports `registerCalcProvider`/`getCalcProvider`/`_resetCalcProvider_forTests` from `core/calcSlot` (Task 9), worker `setCalcProgram` end-to-end (Task 10).
...
- Modify: `packages/kernel/src/velocityGrid.ts` (fold helper + `registerCalcProvider` method + two `resolveColumnTree` call sites + api-literal entry)
- Modify: `packages/kernel/src/types/api.ts` (`registerCalcProvider` signature)
- Modify: `packages/kernel/package.json` (`@wellsfargo-starui/velocity-grid-calc` devDependency)
- Create: `packages/kernel/tests/core/calcSlot.test.ts`
- Create: `packages/kernel/tests/calcColDefFold.test.ts`

**Interfaces:**
- Consumes: nothing from `@wellsfargo-starui/velocity-grid-calc` at runtime (devDep is type-resolution only, for Task 14's bridge integration tests). The provider shape is structural, defined HERE; Tasks 7/8/14 implement to it — **CROSS-TASK CONTRACT** (see phase preamble).
...
- [ ] **Step 3: Write the failing fold test.** `packages/kernel/tests/calcColDefFold.test.ts` drives `resolveColumnTree` through a fold helper (exported for tests from `velocityGrid.ts`? NO — keep it private; test through a small pure function). Put the fold logic in `core/calcSlot.ts` as a pure exported function so it is testable without mounting a grid:
...
- [ ] **Step 4: Hook the two `resolveColumnTree` call sites.** Constructor — current code (`src/velocityGrid.ts:941-942`):
...
`rebuildColumns` — current code (`src/velocityGrid.ts:5217-5219`):
...
→ same `foldCalcColumnDefs(this.options.columnDefs)` substitution. Import `{ registerCalcProvider as slotRegisterCalcProvider, foldCalcColumnDefs, type CalcProviderShape }` next to the existing slot import block (`src/velocityGrid.ts:141-144` currently imports `registerFormatCompiler as slotRegisterFormatCompiler,` and `registerRuleEngine as slotRegisterRuleEngine`).

- [ ] **Step 5: `registerCalcProvider` public method + rebuild trigger.** Insert directly below the existing `registerRuleEngine` — current code (`src/velocityGrid.ts:4941-4948`):
...
api-literal entry next to the existing one — current code (`src/velocityGrid.ts:5633-5634`):
...
Add `"@wellsfargo-starui/velocity-grid-calc": "*",` after `"@wellsfargo-starui/velocity-grid-rules": "*",` (alphabetical: calc sorts before format — place it first in the @cgrid block), then `npm install` at the repo root to refresh the lockfile. Zero RUNTIME imports remains the invariant (final-gates task greps kernel dist).
...
- Modify: `packages/kernel/src/velocityGrid.ts` (fill the Task-9 `onCalcColumnsChanged` seam with program shipping)
...
add the one-line `setCalcProgram(program: WorkerCalcProgram | null): Promise<void>` passthrough. In `velocityGrid.ts`, fill the Task-9 seam — append to `onCalcColumnsChanged()` (and thus to initial registration, which calls it) BEFORE the `updateColumns` dispatch:
...
- Modify: `packages/kernel/src/velocityGrid.ts` (NEW public `getDistinctValues` method + api-literal entry — reality adjustment, see phase preamble item 4)
- Modify: `packages/kernel/src/types/api.ts` (NEW `VelocityGridApi.getDistinctValues` signature)
...
- Produces: `VelocityGridApi.getDistinctValues(colId: string, limit?: number): Promise<string[]>` — the surface `@wellsfargo-starui/velocity-grid-calc`'s typed wrapper (Task 14) and the showcase (Task 15) consume (**this is the "small kernel addition" spec §1.1 item 11 names; the spec's claim that the api method already exists is wrong — only the internal RPC exists**).
...
→ thread `limit`. The one existing internal caller stays as-is (no limit) — current code (`src/velocityGrid.ts:2906-2908`):
...
(set-filter popup keeps the full set — unchanged behavior.) NEW public method on `VelocityGrid` (place near `getColumnFilterType`, `src/velocityGrid.ts:2935-2938`):
...
```bash
grep -rn "@wellsfargo-starui/velocity-grid" /Users/develop/wfh/canvasgrid/packages/calc/src/ ; echo "exit=$?"
```
...
Zero static @wellsfargo-starui/velocity-grid imports in packages/calc/src (grep-verified).
...
- Modify: `apps/cgrid-showcase/package.json` (add `"@wellsfargo-starui/velocity-grid-calc": "*"` dependency)
- Create: `apps/cgrid-showcase/e2e/calculatedColumns.spec.ts`

**Interfaces:**
- Consumes: `wireIntoKernel` from `@wellsfargo-starui/velocity-grid-calc` (Task 14), `wireIntoKernel` from `@wellsfargo-starui/velocity-grid-format` (21c — calc `format` strings compile via the kernel's registered compiler), the kernel Phase E surface (calcSlot synthesis + fold, CalcPass Stages A/B, PREV, program ship).
...
1. **Feature contract** (`src/features/index.ts`): `{ id, label, description, mount(gridHost, controls, theme): VelocityGrid }`. Controls are `.ctrl-btn` / `.ctrl-btn.primary` buttons with `data-testid`; descriptions render into `#desc-bar`; ticking cleanup piggybacks on `grid.destroy()` (`realtimeStomp.ts` pattern, reused by `conditionalStyling.ts`).
...
3. **E2E assertion mechanism**: canvas text is not in the DOM. Calc values are worker-computed and ship in viewport chunks, so the value probe is `grid.getCellValue(rowIndex, colId)` (`velocityGrid.ts:7762` — raw chunk value, the surface `groupSort.spec.ts`/`formatDSL.spec.ts` build on), resolved defs via `grid.columnDefsMap`, and the engine probe `window.__cgridCalc` (mirrors `__cgridRules`). Worker round-trips are async — every post-action assertion polls (`expect.poll` / `waitForFunction`), the `groupSort.spec.ts` pattern.
...
- [ ] **Step 1: Add the `@wellsfargo-starui/velocity-grid-calc` dependency**
...
- package.json: @wellsfargo-starui/velocity-grid-calc workspace dep; features/index.ts:
  registration.
...
- `packages/calc/package.json` — ONLY IF a real build step became necessary during the cycle. It should NOT have: the package ships as TS source (`main`/`exports` → `./src/index.ts`), exactly like `@wellsfargo-starui/velocity-grid-rules`/`@wellsfargo-starui/velocity-grid-format`, and its `build` script stays the no-op echo. Leave untouched.
...
```bash
npm --workspace @wellsfargo-starui/velocity-grid-calc run test:coverage 2>&1 | tail -25
npm --workspace @wellsfargo-starui/velocity-grid run test 2>&1 | tail -3
npm --workspace @wellsfargo-starui/velocity-grid-rules run test 2>&1 | tail -3
...
stat -f%z packages/kernel/dist/velocity-grid.js
```
...
```bash
grep -r "@wellsfargo-starui/velocity-grid-calc" packages/kernel/dist/ ; echo "exit=$?"
grep -rn "@wellsfargo-starui/velocity-grid" packages/calc/src/ ; echo "exit=$?"
```
