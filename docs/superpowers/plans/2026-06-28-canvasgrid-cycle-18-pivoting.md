...
| 3 | **Pivot column synthesis (main)** — merge `pivotColumnTree` into the column order while pivot active; hide primary cols, keep auto-group col; pivot cells read from `pivotValues`; revert cleanly on `setPivotMode(false)`. | 2 | `core/pivotColumns.ts` (new), `cgrid.ts`, `renderer` cell lookup | `pivotColumns.test.ts` + E2E |
| 4 | **Pivot column-group expand/collapse** — collapsed-by-default group headers show the group aggregate; expand reveals child columns; `pivotDefaultExpanded`; hit-testable header affordance; preserve `scrollLeft`. | 4 | `core/subgrid.ts`/column-group state, `renderer/painters`, `cgrid.ts` | `pivotColGroupExpand.test.ts` + E2E |
...
| 6 | **Pivot panel (top-of-grid)** — pills per pivot col, drag header in / pill out / reorder; `pivotPanelShow: always\|onlyWhenPivoting\|never`; shares pivotCols with the tool panel (one list, two views). | 6 | `interaction/pivotPanel/host.ts` (new), `cgrid.ts`, `tokens.css` | `pivotPanel.test.ts` + E2E |
