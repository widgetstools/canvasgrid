...
- cgrid.ts NUL byte: `grep -a` / `sed | tr -d '\000'`; Edit tool fine.
...
**Files:** Modify `core/damageLedger.ts` (+`tests/damageLedger.test.ts` extension), `cgrid.ts` `buildDamageResolveCtx`.
