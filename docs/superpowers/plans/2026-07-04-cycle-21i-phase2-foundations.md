...
1. Recon the strip-host pattern: `RowGroupPanelHost` construction
   (cgrid.ts:1307-1322), `reserveStatusBarSpace` (cgrid.ts:5083),
   `reserveRowGroupPanelSpace` (cgrid.ts:5094), the combined inset
   application (cgrid.ts:5116-5161), `setRowGroupPanelTop`
   (cgrid.ts:5184). Understand how a top strip reserves space so the
   canvas + header + editor overlay reflow.
2. `interaction/toolbarHost.ts`: `ToolbarHost` class, DOM strip mounted
   on `this.root`, full width, `--cg-header-bg` / `--cg-border-color`
   themed, `.cg-toolbar` / `.cg-toolbar-start` / `.cg-toolbar-end`
   classes, tokens.css additions under a `--cg-toolbar-*` namespace.
...
5. Intrinsic end zone: date picker (native `input[type=date]`, value =
   today, `--cg-input-*` themed, focus ring via
   `--cg-input-focus-border`) + save icon button (28px, SVG glyph from
...
7. Destroy path: unmount + inset release, following the sidebar/status
   bar destroy-safety idiom (cgrid.ts:5883-5899 vicinity).
...
1. Recon exact Tier B list in
   ...
   (cgrid.ts:5640 vicinity) accordingly (`getFilterModel`, `getState`,
...
1. `interaction/modalHost.ts`: open/close API, focus trap, ESC +
   backdrop dismiss, `--cg-*` themed, z-order above all chrome,
...
## T5 — `@cgrid/customizer` bootstrap (Lit)

1. Fill the empty scaffold: `lit` + `@lit/context` deps, vite lib
   build + d.ts, lockstep version, `@cgrid/kernel` peer dep, turborepo
   task wiring. /frontend-design before component work.
2. Theming: components consume `--cg-*` tokens directly; one bridge
...
1. Recon `docs/starui-customizer-ui/09-smart-edit.md` + the smart-edit
   option surface shipped in `@cgrid/edit` (Cycle 21g); map doc fields
   to real engine options — missing engine option = fix in
   `@cgrid/edit`, not a UI shim.
...
1. Same shape per `10-bulk-update.md` over `@cgrid/edit` bulk-update
