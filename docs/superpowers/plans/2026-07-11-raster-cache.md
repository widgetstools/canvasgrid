...
- velocityGrid.ts NUL byte: `grep -a` / `tr -d '\000'`; Edit tool fine.
...
**Files:** Modify `apps/cgrid-ext-demo/src/main.ts` (`&noRaster` → `rasterCache: false`, alongside :34–48's params), `e2e/paintInvariance.spec.ts` (new arms: raster-on vs raster-off across the FULL existing step script — boot pair `paintHarness` vs `paintHarness&noRaster` mirroring the :295–393 cache-arm pattern; plus a tick-then-scroll-back step that forces strip patch→consume), extend the live-tick stats spec (scroll phase: `stripHits` grows once warmed, `cellCacheHits/(hits+misses)` reported), `velocityGrid.ts` drain re-eval (measure `BUDGET_MS: 3` vs strip-warmed drain on the OpenFin box in Task 5 — code change here ONLY if kernel tests can lock it deterministically; otherwise record the decision in PERF-NOTES in Task 5 and leave 3).
