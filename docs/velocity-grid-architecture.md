...
8. **Intentionally deferred** — `packages/export` and `packages/excel-pivot` are scaffolds; infinite / tree-data / master-detail / advanced-filter UI are absent from kernel.
...
```
┌─────────────────────────────────────────────────────────────────┐
│ Host: apps / VelocityGridExt (chrome, profiles, providers)      │
└────────────────────────────┬────────────────────────────────────┘
                             │ construct + wire*
┌────────────────────────────▼────────────────────────────────────┐
│ MAIN: VelocityGrid                                              │
...
- Persistence: `statePersistence.ts` + `LocalStorageStateAdapter` (`velocity-grid:state:<gridId>`).
- Companion slices: `moduleState.ts` via `registerStateModule` (templates, rules, edit settings, data-provider).
...
Soft refresh conflation; soft purge on expansion (v1).  
...
4. Persists profile/layout modules that include edit settings, templates, alerts, data-provider selection.

Architecture boundary: **kernel = engine**, **ext = product chrome**, **data/perspective = data plane**.

---

## 10. Key source citations
