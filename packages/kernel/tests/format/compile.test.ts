...
import { compileFormat, compileCompositeColDef } from '../../src/format/compile';
...
describe('compileFormat — Tier 0', () => {
  ...
  it('compiles a currency format with sections', () => {
    ...
    expect(negStyle?.color).toBe('var(--vg-neg-color, #FF7043)');
  });
});
...
describe('compileFormat — Tier 1', () => {
  ...
  it('[if <expr>] selection also drives the section style ([Red] etc.)', () => {
    ...
    expect(big?.color?.toLowerCase()).toBe('var(--vg-neg-color, #ff7043)');
