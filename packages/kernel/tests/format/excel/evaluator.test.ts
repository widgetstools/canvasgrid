...
import { tokenize } from '../../../src/format/tokenizer';
import { parseExcel } from '../../../src/format/excel/parser';
import { evaluateExcel } from '../../../src/format/excel/evaluator';
...
describe('Excel evaluator — Section routing', () => {
  ...
  it('negative value uses second section + applies [Red] style', () => {
    const r = fmt('0.00;[Red]-0.00', -5);
    expect(r.text).toBe('-5.00');
    expect(r.style?.color).toBe('var(--vg-neg-color, #FF7043)');
  });
...
describe('Excel evaluator — Named colors emit StyleObj', () => {
  it('[Green]0.00 emits StyleObj with color', () => {
    ...
    expect(r.style?.color).toBe('var(--vg-pos-color, #14B8A6)');
  });
