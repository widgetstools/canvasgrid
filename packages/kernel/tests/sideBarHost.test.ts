...
describe('SideBarHost', () => {
  ...
  it('detachActivePanel keeps the instance alive; reattachPanel remounts the SAME instance', () => {
    const ctx = makeContext();
    const host = new SideBarHost(root, ctx, { toolPanels: ['columns', 'filters'] });
    host.openPanel('agColumnsToolPanel');
    const inst = host.getInstance('agColumnsToolPanel') as ColumnsRecorder;
    ...
    const detached = host.detachActivePanel();
    expect(detached).not.toBeNull();
    expect(detached!.id).toBe('agColumnsToolPanel');
    expect(detached!.gui).toBe(inst.getGui());
    // The panel area collapsed (mirrors closePanel's visuals) but the
    // instance itself was NOT destroyed.
    const bar = root.querySelector('.cg-side-bar') as HTMLElement;
    const panelHost = bar.querySelector('.cg-side-bar-panel') as HTMLElement;
    expect(panelHost.style.display).toBe('none');
    expect(panelHost.querySelector('.cg-tool-panel-recording')).toBeNull();
    expect(inst.destroyCount).toBe(0);
    expect(host.getOpenedToolPanelId()).toBeNull();
    expect(host.getDetachedId()).toBe('agColumnsToolPanel');
    const tab = bar.querySelector('.cg-side-bar-tab[data-cg-detached]') as HTMLElement | null;
    expect(tab).not.toBeNull();
    expect(tab!.getAttribute('aria-pressed')).toBe('false');
    // The live instance is still reachable + unchanged.
    expect(host.getInstance('agColumnsToolPanel')).toBe(inst);

    const ok = host.reattachPanel();
    expect(ok).toBe(true);
    expect(host.getDetachedId()).toBeNull();
    expect(host.getOpenedToolPanelId()).toBe('agColumnsToolPanel');
    expect(panelHost.querySelector('.cg-tool-panel-recording')).not.toBeNull();
    expect(bar.querySelector('.cg-side-bar-tab[data-cg-detached]')).toBeNull();
    // SAME instance — never destroyed/recreated across the detach/reattach.
    expect(host.getInstance('agColumnsToolPanel')).toBe(inst);
    expect(inst.destroyCount).toBe(0);
    host.destroy();
  });

  it('destroyDetached tears down the detached instance without touching the sidebar open state', () => {
    const ctx = makeContext();
    const host = new SideBarHost(root, ctx, { toolPanels: ['columns'] });
    host.openPanel('agColumnsToolPanel');
    const inst = host.getInstance('agColumnsToolPanel') as ColumnsRecorder;
    host.detachActivePanel();
    expect(inst.destroyCount).toBe(0);
    host.destroyDetached();
    expect(inst.destroyCount).toBe(1);
    expect(host.getDetachedId()).toBeNull();
    expect(host.getInstance('agColumnsToolPanel')).toBeNull();
    host.destroy();
  ...
  it('clicking a detached tab calls onReattachRequest instead of toggling open/close', () => {
    const ctx = makeContext();
    const reattachRequests: string[] = [];
    const host = new SideBarHost(root, {
      ...ctx,
      onReattachRequest: (id) => reattachRequests.push(id),
    }, { toolPanels: ['columns', 'filters'] });
    host.openPanel('agColumnsToolPanel');
    host.detachActivePanel();
    const bar = root.querySelector('.cg-side-bar') as HTMLElement;
    const colsTab = bar.querySelectorAll('.cg-side-bar-tab')[0] as HTMLElement;
    colsTab.dispatchEvent(new MouseEvent('click', { bubbles: true }));
    expect(reattachRequests).toEqual(['agColumnsToolPanel']);
    // Detach state is untouched — the click did NOT open/close anything.
    expect(host.getDetachedId()).toBe('agColumnsToolPanel');
    expect(host.getOpenedToolPanelId()).toBeNull();
    host.destroy();
  });

  it('detach/reattach does not interfere with normal open/close of OTHER panels', () => {
    const ctx = makeContext();
    const host = new SideBarHost(root, ctx, { toolPanels: ['columns', 'filters'] });
    host.openPanel('agColumnsToolPanel');
    host.detachActivePanel(); // columns is now floating
    // Filters opens/closes normally while columns sits detached.
    host.openPanel('agFiltersToolPanel');
    expect(host.getOpenedToolPanelId()).toBe('agFiltersToolPanel');
    host.closePanel();
    expect(host.getOpenedToolPanelId()).toBeNull();
  ...
  it('destroy() also destroys a detached instance so it does not leak', () => {
    const ctx = makeContext();
    const host = new SideBarHost(root, ctx, { toolPanels: ['columns'] });
    host.openPanel('agColumnsToolPanel');
    const inst = host.getInstance('agColumnsToolPanel') as ColumnsRecorder;
