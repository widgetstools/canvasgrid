...
import { ledgerExposedBand } from './retainedBand';
...
export interface DamageResolveCtx {
  ...
  /** `true` when the retained paint-cache layer is actively serving this
   *  frame (paintCache enabled AND the layer's `available`). This selects
   *  which strategy of the retained-pixel owner holds the frame's still-valid
   *  pixels — the retained layer, or the renderer's on-screen scroll self-blit
   *  (`Renderer.paint`'s `damage.blit` branch). The ledger does not interpret
   *  the flag itself: it hands this context to `ledgerExposedBand`
   *  (`retainedBand.ts`), the one place that maps strategy → "is the exposed
   *  band mine to repaint?".
   *
   *  Sticky-band + pinned/totals chrome redamage stays unconditional under
   *  either strategy (see below) — those are genuinely screen-anchored chrome
   *  the layer never covers, and the cache-off pipeline already redamages them
   *  every scroll tick. Defaults to falsy, so a caller that knows nothing
   *  about the layer gets the scroll-blit strategy and the historical
   *  cache-off resolution byte-for-byte. */
...
export class DamageLedger {
  ...
  takeResolved(ctx: DamageResolveCtx): ResolvedDamage {
    ...
    if (dy !== 0) {
      blit = { dy };
      // Collapse target #2 — ask the single retained-pixel owner whether
      // this frame's exposed band is the ledger's to repaint, instead of
      // re-deriving the band here behind a `paintCacheLayerActive` gate.
      // Under the retained-layer strategy the answer is `null`: the layer's
      // own `planLayer` shift/reset already schedules a raster for exactly
      // the band it invalidated, so a second push here would force a
      // redundant per-tick raster and a plain in-layer scroll tick must
      // resolve to an EMPTY `dataRects` (present-only frame). Under the
      // scroll-blit strategy the ledger IS the owner and gets the band.
      const exposed = ledgerExposedBand(ctx, dy);
      if (exposed !== null) pushData(exposed.top, exposed.bottom);
