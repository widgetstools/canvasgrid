...
export interface ViewportState {
  ...
  firstRow: number;
  lastRow: number;
  /** The UNPADDED first visible data row, i.e. `firstRow` before `overscan`
   *  widens it downward. Threaded to the worker as `stickyBoundaryRow` so the
   *  sticky-ancestor band tracks the real on-screen scroll position rather
   *  than the (overscan-widened) fetch window.
   *
   *  The row overscan is deliberately wide enough to cover the retained
   *  paint-cache layer's coverage (see `ViewportManager`'s fetch-window
   *  coupling), which grows the gap between "first fetched row" and "first
   *  VISIBLE row". The worker's `computeStickyAncestors` was originally keyed
   *  off the FETCHED window's `rowStart` as a proxy for "first row rendered on
   *  screen", so any scroll depth smaller than the overscan buffer left
   *  `rowStart` at 0 even though the user had genuinely scrolled a group's
   *  header off screen — and the sticky band silently stopped appearing.
   *  Exposing the true unpadded boundary decouples the visual sticky-pin
   *  decision from the fetch window's padding entirely.
   *
   *  REQUIRED (legacy declared this optional purely so hand-built
   *  `ViewportState` test fixtures kept compiling; every real
   *  `computeViewport()` result has always set it, and no test in this
   *  package's parity gate builds a `ViewportState` by hand). */
   ...
   *  when no `FloatingFilterSubgrid` is in the stack. */
...
export interface ViewportInput {
   ...
   *
   *  SPEC.md §2 lists this as dead code to drop in favour of `rowBuffer`. It
   *  stays because the ported parity suites (`viewport`, `totalsSubgrid`,
   *  `pinnedRows`) drive `computeViewport` through it directly in ~30 cases,
   *  so removing it would mean editing fixtures — which the parity gate
   *  forbids. Nothing in the runtime path writes it any more: `ViewportManager`
   *  only ever passes `rowBuffer`. */
  overscanRows?: number;
  ...
  /** Cumulative-height index for the data subgrid. When supplied AND its
   *  length matches the data subgrid's row count, the first-visible-row search
   *  descends through the index in O(log n) and `dataContentHeight` reads
   *  `index.totalHeight()` exactly. When omitted the viewport falls back to
   *  the uniform-height approximation built from `subgrid.getRowHeight(0)`. */
...
export function computeViewport(opts: ViewportInput): ViewportState {
  ...
  // the scrollable data region. Headers and the floating-filter row ride
  // here by default. A top-pinned totals row does too when its subgrid is
  // stacked before the data subgrid.
  ...
  // each data subgrid exposes `getRowHeight(local)`. When a
  // `dataRowHeightIndex` is supplied the first/last visible-row search
  // descends the Fenwick tree in O(log n) and the pre-window top is read as a
  // single `index.topOf(firstDataRow)` query. Without the index we fall back
  // to the uniform-height approximation (`subgrid.getRowHeight(0)` as the
  // fallback row size). Within the visible range we still walk per-row so
  // variable-height rows position correctly relative to one another (no
  // overlap, no gap).
