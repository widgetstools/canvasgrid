// Owns the scroll/viewport subsystem: scroll-state fields, the native
// `'scroll'` listener (routed through the host's DisposableRegistry),
// `computeCurrentViewport` + `syncSizer`, the velocity sampler used by the
// prefetch-range expansion, the request-coalescing flags around the worker
// viewport fetch, and the `setScroll` / `ensure*Visible` scroll-imperative
// helpers.
//
// The grid is the thin delegator: `recompute()` returns the new state, which
// the grid assigns back to its own read-only `viewport` field so the many
// readers of `viewport.visibleColumns` etc. don't have to change. The
// `dispatchViewportRequest` dep is the worker-pipeline seam: ViewportManager
// owns coalescing, prefetch-range math, and the `request` entry point; the
// data plane owns the worker call + chunk plumbing. The split keeps this
// module pure-viewport and surfaces the worker-pipeline coupling instead of
// dragging half the grid in.
...
export interface ViewportManagerDeps {
  ...
  dispatchViewportRequest(opts: {
    ...
    rowEnd: number;
    columns: string[];
    /** The true on-screen first-visible row (unpadded by row overscan), so
     *  the worker's sticky-ancestor computation tracks the real scroll
     *  position rather than the overscan-widened fetch window. See
     *  `ViewportState.firstVisibleDataRow`'s doc for the full regression
     *  story. */
...
export class ViewportManager {
  ...
  private dispatchRequest(): void {
    ...
    const stickyBoundaryRow = this.currentState.firstVisibleDataRow;
