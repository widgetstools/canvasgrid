...
// Group header decoration resolves through the SAME helper the leaf path in
// `resolveColDef` uses. The legacy kernel inlined a duplicate of this logic
// here to dodge an import cycle with `propertyChain.ts`; extracting the
// shared bottom (`core/headerDecoration.ts`) removes both the cycle and the
// duplicate.
  ...
  function walk(
    ...
    if (isColGroupDef(node)) {
      ...
      const groupNode: ResolvedColGroupDef = {
        ...
        // Raw value retained for round-trip / introspection.
        headerClass: node.headerClass,
        // Pre-resolved static/fn slots for headerClass + headerStyle. Same
        // helper as the leaf path in `resolveColDef`.
        ...resolveHeaderDecoration(node.headerClass, node.headerStyle),
        depth,
        children: [],
        leafColIds: [],
      };
