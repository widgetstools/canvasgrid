import type {
  ...
  CellClass, CellClassRules, CellStyleFunc,
...
// Leaf header decoration (headerClass / headerStyle) resolves through the
// SAME helper the group path in `columnTree.ts` uses — see
// `core/headerDecoration.ts` for why it lives in its own module.
import { resolveHeaderDecoration, type HeaderClassFn } from './headerDecoration';
...
export interface ResolvedColDef<TRow = any> {
  ...
  cellIcon?: (params: CValueFormatterParams<TRow, unknown>) => import('@wellsfargo-starui/vg-new-engines').IconRef | null;
  /** Cycle 28 — leaf-header prefix/suffix icon. Always a fn after resolve —
   *  static IconRef forms are wrapped by `normalizeHeaderIcon`. */
  headerIcon?: (params: { colId: string }) => import('@wellsfargo-starui/vg-new-engines').IconRef | null;
...
function compileFormatSlots<TRow>(
  ...
  if (typeof merged.valueFormatter === 'string') {
    ...
    return {
      ...
      cellIcon: (() => {
        const staticRef: import('@wellsfargo-starui/vg-new-engines').IconRef | null =
        ...
        return (p: CValueFormatterParams<TRow, unknown>) =>
          (evalFormatProgram(program, p).icon as import('@wellsfargo-starui/vg-new-engines').IconRef | null) ?? staticRef;
      })(),
    } as CColDef<TRow>;
  }
...
export function resolveColDef<TRow>(
  ...
  return {
    ...
    cellStyleFn: typeof compiledMerged.cellStyle === 'function'
      ...
      : undefined,
    // Pre-compile cellClass + cellClassRules. Cycle 6 / Task 7.
    ...
    // Header decoration (headerClass static/fn + headerStyle static/fn) —
    // resolved by the shared helper that `resolveColumnTree` also calls for
    // GROUP header defs, so the two paths cannot drift.
    ...resolveHeaderDecoration(merged.headerClass, merged.headerStyle),
...
function normalizeHeaderIcon(
  v: import('@wellsfargo-starui/vg-new-engines').IconRef | ((params: { colId: string }) => import('@wellsfargo-starui/vg-new-engines').IconRef | null) | undefined,
): ((params: { colId: string }) => import('@wellsfargo-starui/vg-new-engines').IconRef | null) | undefined {
  if (v === undefined) return undefined;
