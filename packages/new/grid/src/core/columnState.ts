...
/** Membership + ordinal of one leaf in one ORDERED role list
 *  (`rowGroupColumns` / `pivotColumns`). */
interface OrderedRoleSlot {
  active: boolean;
  index: number | null;
}

/**
 * The shared "runtime provider wins, else def slot" rule for an ordered
 * role list. `rowGroup`/`rowGroupIndex` and `pivot`/`pivotIndex` are
 * resolved identically; the legacy kernel spelled the block out twice.
 *
 * When `runtimeIndexById` is `null` the caller threaded NO runtime provider
 * for this role, so the static def slots are authoritative (back-compat).
 * When it is present it is authoritative in BOTH directions — a column
 * absent from the runtime list reads `active: false, index: null` and the
 * def slots are ignored. That asymmetry with `aggFunc` below is deliberate:
 * an ordered role list is always fully materialised by its runtime state
 * primitive, so absence there really does mean "not in this role".
 */
function resolveOrderedRole(
  runtimeIndexById: ReadonlyMap<string, number> | null,
  colId: string,
  defActive: boolean | undefined,
  defIndex: number | null | undefined,
): OrderedRoleSlot {
  if (runtimeIndexById === null) {
    return { active: defActive ?? false, index: defIndex ?? null };
  }
  const i = runtimeIndexById.get(colId);
  return { active: i !== undefined, index: i ?? null };
}

/**
 * Same rule for `aggFunc`, with ONE documented divergence: a runtime MISS
 * falls THROUGH to `def.aggFunc` instead of resolving to `null`.
 *
 * This is load-bearing, not tidiness. The value-column registry is opt-in
 * (populated by `addValueColumn` / `setValueColumnAggFunc`), so an app that
 * declares `aggFunc` on its colDefs and never touches the pivot API has an
 * EMPTY registry. Resolving misses to `null` exported `aggFunc: null` for
 * every column and wiped the colDef aggregates on the next profile /
 * columnState round-trip — blanking SSRM group totals.
 *
 * The nullish chain below is exactly that: registry hit → def slot → null,
 * with `runtimeAggFuncById === null` (no provider threaded at all) short-
 * circuiting straight to the def slot.
 */
function resolveAggFuncRole(
  runtimeAggFuncById: ReadonlyMap<string, string> | null,
  colId: string,
  defAggFunc: string | undefined,
): string | null {
  return runtimeAggFuncById?.get(colId) ?? defAggFunc ?? null;
}
...
export function snapshotState(
  ...
  // A `null` index means "no runtime provider for this role" and selects
  // the def-slot fallback inside the resolvers above; an EMPTY map means
  // "provider present, list empty" — a different thing entirely.
  const indexList = (ids: readonly string[] | undefined): ReadonlyMap<string, number> | null => {
    if (!ids) return null;
    const out = new Map<string, number>();
    ids.forEach((id, i) => out.set(id, i));
    return out;
  };
  const rowGroupIndexById = indexList(runtime?.rowGroupColumns);
  const pivotIndexById = indexList(runtime?.pivotColumns);
  const aggFuncById: ReadonlyMap<string, string> | null = runtime?.valueColumns === undefined
    ? null
    : new Map(runtime.valueColumns.map((v) => [v.colId, v.aggFunc]));
  ...
  for (const leaf of tree.leaves) {
    ...
    const rowGroup = resolveOrderedRole(
      rowGroupIndexById, leaf.colId, def.rowGroup, def.rowGroupIndex,
    );
    const pivot = resolveOrderedRole(
      pivotIndexById, leaf.colId, def.pivot, def.pivotIndex,
    );
    const aggFunc = resolveAggFuncRole(aggFuncById, leaf.colId, def.aggFunc);
...
function applySlot(
  ...
  // sort — UNCONDITIONAL by design. Unlike width / hide / pinned above,
  // this branch is NOT a diff: every slot that mentions `sort` at all
  // (including `sort: null`) emits a record, and `oldValue` is always
  // `null`.
  //
  // WHY: sort direction does not live on the leaf's ResolvedColDef, so
  // there is no per-column "old value" to diff against — the authoritative
  // sort state is the grid-level SortModel. The consumer
  // (`ColumnStateManager`) does not treat these records as deltas either;
  // it rebuilds the ENTIRE SortModel from the union of every sort record in
  // the batch, ordered by `sortIndex`, dropping `direction: null` entries.
  // A record suppressed here because "it looks unchanged" would silently
  // drop that column from the rebuilt model, and a `sort: null` entry has
  // to survive precisely so it can be dropped from the union — that is how
  // clearing a column's sort is expressed.
  //
  // `oldValue: null` is therefore a placeholder, not a reading of prior
  // state. It is part of the observable contract (tests pin it); do not
  // "fix" it by threading the SortModel in here.
