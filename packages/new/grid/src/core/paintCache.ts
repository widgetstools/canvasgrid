...
import { layerExposedBand } from './retainedBand';
...
export function planLayer(args: {
  ...
  // Collapse target #2 — the band this shift exposed comes from the shared
  // retained-pixel derivation, in CONTENT space and clamped to the data
  // extent so a shift near either content edge yields an empty band rather
  // than a negative-height one. The renderer's on-screen scroll blit is the
  // other strategy of the same owner and gets its band from the identical
  // call, in screen space.
  const band = layerExposedBand({ dy, layerTop: newTop, layerHeight, contentHeight });
  const rasterBands = band !== null ? [band] : [];
