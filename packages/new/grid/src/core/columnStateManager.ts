...
/**
 * SPEC collapse target #4 — the two column trees that coexist under pivot
 * mode, NAMED rather than remembered.
 *
 * The legacy kernel re-derived the choice inline at three separate call
 * sites (`getColumnState`, `applyColumnStateInternal`,
 * `applyRoleStateFromColumnState`), each spelling out the same
 * `isPivotActive() && getPrimaryColumnTree() !== null ? primary! : live`
 * ternary. Three copies of an invariant is three chances to forget it; this
 * type + `selectStateTree` below make it one typed decision.
 */
export interface ColumnModelTrees {
  /** The tree that is RENDERED. Under pivot mode this is the SYNTHESIZED
   *  pivot result tree (`pivotcol*` leaves + the auto-group column); with
   *  pivot off it is the user's own tree. */
  display: ColumnTree;
  /** The user's own pre-pivot tree, cached by the pivot engine while pivot
   *  is active. `null` when there is nothing cached — either pivot is off
   *  (in which case `display` already IS the primary tree) or the cache has
   *  not been populated yet. */
  primary: ColumnTree | null;
  /** Whether pivot mode is currently engaged. */
  pivotActive: boolean;
}

/** Which of the two trees a column-state operation resolved to. */
export type ColumnTreeRole = 'primary' | 'display';

export interface StateTreeSelection {
  role: ColumnTreeRole;
  tree: ColumnTree;
}

/**
 * The pivot-primary invariant, in one place (AG-Grid parity, Cycle 18 /
 * Task 9): saved column state ALWAYS describes primary columns, so every
 * snapshot / validate / mutate path must run against the primary tree
 * whenever pivot mode has cached one. Snapshotting the synthesized tree
 * instead would drop every primary column from the save, and the round-trip
 * restore could then never reinstate the pivot / value role lists.
 *
 * The `primary !== null` guard is load-bearing: pivot can be flagged active
 * before the engine has cached a primary tree (mid-activation), and in that
 * window the live tree is still the user's own columns.
 */
export function selectStateTree(model: ColumnModelTrees): StateTreeSelection {
  if (model.pivotActive && model.primary !== null) {
    return { role: 'primary', tree: model.primary };
  }
  return { role: 'display', tree: model.display };
}
...
export class ColumnStateManager<TRow = unknown> {
  ...
  /** Snapshot the deps' two-tree view into the typed `ColumnModelTrees`
   *  shape. Read fresh on every call — `rebuildColumns` swaps the live tree
   *  out from under us mid-`applyColumnStateInternal`. */
  private columnModelTrees(): ColumnModelTrees {
    return {
      display: this.deps.getColumnTree(),
      primary: this.deps.getPrimaryColumnTree(),
      pivotActive: this.deps.isPivotActive(),
    };
  }

  /** The tree every column-state read / validate / mutate path must bind
   *  to. See `selectStateTree` for the pivot-primary invariant. */
  private stateTree(): StateTreeSelection {
    return selectStateTree(this.columnModelTrees());
  }
  ...
  private applyColumnStateInternal(
    ...
    // Cycle 18 / Task 9 — apply validates + mutates against the tree
    // `selectStateTree` picks: under pivot mode that is the PRIMARY tree,
    // because the live tree describes the synthesized pivot result
    // (pivotcol* leaves + the auto-group column) while saved state always
    // describes primary columns. The role fan-out in
    // `applyRoleStateFromColumnState` then drives the pivot synthesis to
    // re-build from the restored primary state.
    const applyTree = this.stateTree().tree;
    ...
    if (leafOrderChanged) {
      ...
      // ── INTENTIONAL DOUBLE APPLY — DO NOT COLLAPSE ────────────────────
      // SPEC §2 "known workarounds to preserve". `rebuildColumns()` just
      // re-resolved the tree from `options.columnDefs`, and those raw
      // `CColDef`s do NOT carry the runtime width / hide / pinned mutations
      // `applyStateToTree` wrote a few lines above — those landed on the
      // ResolvedColDef objects of the tree that the rebuild has now
      // discarded. Without this second pass the restore silently loses
      // every width and hide flip whenever the leaf order also changed
      // (symptom: the demo's Restore button needed two clicks).
      //
      // Deliberately NOT re-collecting changes: `result.changes` from the
      // FIRST pass stays the authoritative change log, so the event
      // fan-out below reports each mutation exactly once. The return value
      // here is discarded on purpose.
  ...
  private applyRoleStateFromColumnState(state: readonly CColumnState[]): void {
    ...
    // Cycle 18 / Task 9 — under pivot mode the live tree covers synthesized
    // leaves only; validate against the tree `selectStateTree` picks so
    // saved state carrying primary col ids round-trips.
    const validLeafById = this.stateTree().tree.leafById;
