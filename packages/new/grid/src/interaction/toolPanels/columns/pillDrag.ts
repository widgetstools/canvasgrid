...
import { beginDrag, makeInsertionLine } from '../../dragController';
import {
  getZoneRect,
  isPointInRect,
  makeDragGhost,
...
export function beginPillDrag(opts: BeginPillDragOptions): void {
  ...
  const insertionLine = makeInsertionLine('vg-columns-panel-insertion-line');
  ...
  const mountInsertionLine = (clientY: number): void => {
    ...
    insertionLine.showAt(content, target.gapY - zoneRect.top);
  };
