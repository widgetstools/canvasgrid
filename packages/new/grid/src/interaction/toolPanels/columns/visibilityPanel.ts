...
import { beginDrag } from '../../dragController';
import {
  getZoneRect,
  isPointInRect,
  makeDragGhost,
...
export class ColumnVisibilityPanel {
  ...
  private beginRowDrag(e: MouseEvent, id: string, kind: 'col' | 'group'): void {
    ...
    // Last observed pointer position — the Escape/abort teardown needs a
    // coordinate to push into `setColumnHeaderDragHover(null, …)`.
    let lastX = e.clientX;
    let lastY = e.clientY;
    ...
    // Teardown is separated from the drop verdict so the DragController can
    // run it on Escape / an aborted gesture too. Guarded because the
    // controller also fires `onCancel` after `onDrop`, and the router
    // clear-hover calls are observable (they push a null hover downstream).
    let tornDown = false;
    const teardown = (x: number, y: number): void => {
      if (tornDown) return;
      tornDown = true;
      ...
      if (hasColHeaderDropRouter) {
        (api as any).setColumnHeaderDragHover(null, x, y);
      }
    };

    const onUp = (ev: MouseEvent) => {
      // Legacy tore the decorations down BEFORE committing so the commit
      // observes a clean hover state; preserve that order exactly.
      teardown(ev.clientX, ev.clientY);
    ...
    beginDrag(e, {
      ...
      onDragMove: (ev) => onMove(ev as MouseEvent),
      onDrop: (ev) => onUp(ev as MouseEvent),
      // A press that never crossed the threshold still has to clear the
      // press-time decoration; legacy did this in the same `onUp`.
      onClickRelease: (ev) => teardown(ev.clientX, ev.clientY),
      onCancel: () => teardown(lastX, lastY),
    });
  }
