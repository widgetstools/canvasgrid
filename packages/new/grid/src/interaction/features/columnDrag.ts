...
// The grid's single drag budget. The gesture lifecycle for a header drag is
// owned by `FeatureChain` (canvas mousedown → window mousemove/mouseup),
// which runs on the same `DragController`; this feature only applies the
// threshold to decide when a press becomes a reorder.
import { DRAG_THRESHOLD_PX } from '../dragController';
