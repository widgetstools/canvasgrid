...
/** What this panel needs to remember for the lifetime of a pill gesture.
 *  The gesture skeleton itself lives in `DragController`. */
interface DragState {
  colId: string;
  fromIndex: number;
  pillEl: HTMLElement;
...
export class PivotPanelHost {
  ...
  /** In-flight gesture handle from the shared `DragController`. */
    ...
    const session = beginDrag(e, {
      ...
      onDrop: (ev) => {
        if (this.isPointInPanel(ev.clientX, ev.clientY)) {
          ...
          const moved = this.ctx.tryCrossPanelMove?.(state.colId, ev.clientX, ev.clientY) ?? false;
