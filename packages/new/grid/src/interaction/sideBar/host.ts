...
export class SideBarHost {
  ...
  /** In-flight splitter gesture from the shared `DragController`. */
  ...
  private handleDragStart(e: MouseEvent): void {
    ...
    // The splitter tracks the pointer immediately (no threshold), and the
    // gesture has no commit step beyond the live width writes, so `onCancel`
    // is the single end path.
    this.dragSession = beginDrag(e, {
      family: 'mouse',
      thresholdPx: 0,
      onDragMove: (ev) => this.handleDragMove(ev as MouseEvent),
      onCancel: () => {
        this.dragSession = null;
        this.handleDragEnd();
      },
