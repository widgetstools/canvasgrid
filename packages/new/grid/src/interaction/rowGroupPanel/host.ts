...
/** What this panel needs to remember for the lifetime of a chip gesture.
 *  The press/drag phase, the pointer-id match, the capture and the listener
 *  lifecycle all live in `DragController` now — only the payload is local. */
interface DragState {
  colId: string;
  fromIndex: number;
  pillEl: HTMLElement;
...
export class RowGroupPanelHost {
  ...
  /** In-flight gesture handle from the shared `DragController`. `null`
   *  between gestures. */
  ...
  private handlePointerDown(
    ...
    if (this.destroyed) return;
    this.cancelDrag();
    const state: DragState = { colId, fromIndex: index, pillEl };
    // Left mouse / primary touch only — right-clicks open the column
    // context menu (Cycle 10), middle-clicks are reserved. The
    // controller enforces the button check and returns null otherwise.
    const session = beginDrag(e, {
      family: 'pointer',
      // Capture so a release outside the chip / panel still fires the
      // matching pointerup on this gesture.
      captureOn: pillEl,
      onDragStart: (ev) => {
        this.dragState = state;
        this.mountGhost(pillEl, ev.clientX, ev.clientY);
      },
      onDragMove: (ev) => {
        this.positionGhost(ev.clientX, ev.clientY);
        if (this.isPointInPanel(ev.clientX, ev.clientY)) {
          ...
          this.updateInsertionLine(ev.clientX);
        } else {
          this.clearDragHover();
        }
      },
      onDrop: (ev) => {
        ...
        if (this.isPointInPanel(ev.clientX, ev.clientY)) {
          const targetSlot = this.updateInsertionLine(ev.clientX);
          ...
          this.ctx.tryCrossPanelMove?.(state.colId, ev.clientX, ev.clientY);
        }
      },
      onCancel: () => {
   ...
   *  ghost, and (via the controller) the window-level listeners.
   *  Idempotent. */
  private cancelDrag(): void {
    const session = this.dragSession;
    this.dragSession = null;
    if (session) {
      // Runs the shared teardown, which calls back into `onCancel`.
      session.cancel();
      return;
    }
    this.dragState = null;
