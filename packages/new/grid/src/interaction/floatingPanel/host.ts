...
export class FloatingPanelHost {
  ...
  /** In-flight gestures from the shared `DragController`. */
  private dragSession: DragSession | null = null;
  private resizeSession: DragSession | null = null;
  ...
  close(): void {
    ...
    // Drop the gestures BEFORE cancelling so the controller's `onCancel`
    // can't schedule a rect-change for a panel that is closing.
    this.dragGesture = null;
    this.resizeGesture = null;
    this.dragSession?.cancel();
    this.resizeSession?.cancel();
  ...
  private handleDragStart(e: PointerEvent): void {
    ...
    // Panel move has no threshold — the frame tracks the pointer from the
    // first move. `onCancel` is the single end path, matching legacy where
    // pointerup and pointercancel both routed to `handleDragEnd`.
    const session = beginDrag(e, {
      family: 'pointer',
      thresholdPx: 0,
      captureOn: this.titlebar,
      preventDefaultOnDown: true,
      onDragMove: (ev) => this.handleDragMove(ev as PointerEvent),
      onCancel: () => {
        this.dragSession = null;
        this.handleDragEnd();
      },
    ...
    if (!session) {
      // Not a primary-button press — drop the gesture we speculatively
      // recorded so a later move can't act on it.
      this.dragGesture = null;
      return;
    }
  ...
  private handleResizeStart(e: PointerEvent): void {
    ...
    // Keep the titlebar drag from also claiming this press.
    e.stopPropagation();
    this.resizeGesture = {
      pointerId: e.pointerId,
      startClientX: e.clientX,
    ...
    const session = beginDrag(e, {
      family: 'pointer',
      thresholdPx: 0,
      captureOn: this.resizeHandle,
      preventDefaultOnDown: true,
      onDragMove: (ev) => this.handleResizeMove(ev as PointerEvent),
      onCancel: () => {
        this.resizeSession = null;
