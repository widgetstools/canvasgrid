...
import { fillGlyph, strokeIconPath } from '../icons';
...
function drawCellIcon(gc: CachedContext2D, icon: PendingCellIcon): void {
  ...
  if (icon.path === null) {
    ...
    fillGlyph(gc, icon.emoji!, icon.x + icon.size / 2, icon.y + icon.size / 2, icon.size);
  } else {
    strokeIconPath(gc, icon.path, icon.x, icon.y, icon.size, icon.tint);
  }
  gc.cache.restore();
}

/**
 * Claim an inline icon slot inside a cell: the icon sits at the leading or
 * trailing edge and the text shifts away from it via `config.padding` (every
 * built-in painter reads `padding.left/right` with its own default).
 *
 * One implementation for both inline-icon sites — the data-cell / rule
 * indicator icon and the leaf-header `headerIcon` — which differ only in the
 * vertical slot they occupy: a data cell uses the row band, while a span-height
 * header cell uses its extended rect (`cellTop`..`row.bottom`) but sizes off
 * the SHORTER of the two so a tall spanning header doesn't get an oversized
 * glyph. Mutates `config.padding`; returns the icon to draw after the painter.
 */
function placeInlineIcon(
  icon: ResolvedDrawableIcon,
  col: ViewportColumn,
  slotTop: number,
  slotHeight: number,
  sizeBasis: number,
  ...
  const iconSize = Math.floor(sizeBasis * 0.55);
  const leading = icon.position === 'leading';
  const pending: PendingCellIcon = {
    path: icon.path,
    emoji: icon.emoji,
    x: leading
      ? col.left + CELL_ICON_EDGE_PAD
      : col.left + col.width - CELL_ICON_EDGE_PAD - iconSize,
    y: slotTop + (slotHeight - iconSize) / 2,
    size: iconSize,
    tint: icon.color ?? config.fg,
  };
  ...
  for (let ri = 0; ri < rows.length; ri++) {
    ...
    for (const col of cols) {
      ...
      if (ruleIconRef !== null || (row.subgrid.isData && !isFooterRow[r])) {
        ...
        if (drawable !== null) {
          pendingIcon = placeInlineIcon(drawable, col, row.top, row.height, row.height, config);
        }
      }
      ...
      if (
        ...
        const drawable = resolveDrawableIcon(iconRef);
        if (drawable !== null) {
          const boundsH = row.bottom - cellTop;
          pendingIcon = placeInlineIcon(
            drawable, col, cellTop, boundsH, Math.min(row.height, boundsH), config,
          );
        }
      }
