...
export class WorkerClient {
  ...
  /** Fetch one viewport chunk.
   *
   *  In-flight requests are NEVER cancelled: the worker answers every
   *  `getViewport`, including ones the model has moved past, so this promise
   *  always settles. `visibleModelGeneration` is the worker's id for the
   *  visible order the chunk describes — the caller's own viewport
   *  generation/epoch decides whether to keep the chunk or drop it.
   *  `undefined` from a worker that predates the field means "unknown".
   *  See `worker/visibleModel.ts`. */
  getViewport(req: ViewportRequest): Promise<{
    chunk: ViewportChunk;
    ...
    return this.send<{
      ...
      stickyAncestors?: StickyAncestor[];
      visibleModelGeneration?: number;
    }>({ type: 'getViewport', payload: req })
      .then((r) => ({
        chunk: normalizeViewportChunk(r.chunk),
        stickyAncestors: r.stickyAncestors,
        visibleModelGeneration: r.visibleModelGeneration,
      }));
  }
