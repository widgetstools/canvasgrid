...
export interface State {
  ...
  /** Monotonic id for the CURRENT visible order. Advanced by every
   *  mutation that invalidates `visibleCache` — see `visibleModel.ts`,
   *  which owns both the invalidation set and the `getViewport` staleness
   *  contract this counter makes explicit. Stamped onto each `viewport`
   *  response so the main thread's epoch guard has something authoritative
   *  to compare against. */
