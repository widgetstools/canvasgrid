...
import { visibleModelGeneration } from '../visibleModel';
...
export async function handleViewport(
  ...
  switch (req.type) {
    ...
    case 'getViewport': {
      // No cancellation by design — see `worker/visibleModel.ts`. The await
      // below can park on a mid-pipeline round-trip while a model mutation
      // lands; this request still runs to completion and still answers,
      // because the client's pending map is keyed by `id` and would
      // otherwise never resolve. The generation stamped on the response is
      // how the main thread tells a superseded chunk from a current one.
      const visIds = await helpers.visibleAsync();
      const generation = visibleModelGeneration(state);
      ...
      post(
        { id: req.id, type: 'viewport', chunk, stickyAncestors, visibleModelGeneration: generation },
