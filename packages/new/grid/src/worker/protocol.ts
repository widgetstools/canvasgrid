...
export type WorkerResponse =
  ...
  | {
      ...
      /** Generation of the visible order this chunk was sliced from.
       *
       *  `getViewport` has NO cancellation: the request awaits mid-pipeline
       *  (external filter / postSortRows round-trips), so the model can be
       *  invalidated underneath it — and the worker answers anyway, because
       *  `WorkerClient` correlates by `id` through a pending map and a
       *  missing response strands that promise forever. The main thread owns
       *  the discard decision via its own viewport generation/epoch.
       *
       *  This field is what makes that contract checkable instead of
       *  implicit: compare it across responses (or against the generation
       *  observed when the request was issued) to tell a current chunk from
       *  a superseded one. Optional — a chunk from a worker that predates
       *  the field decodes as `undefined`, which means "unknown", and the
       *  main thread's epoch guard remains authoritative either way.
       *  See `worker/visibleModel.ts` for the full contract. */
      visibleModelGeneration?: number;
    }
