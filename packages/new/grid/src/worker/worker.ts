...
export function createWorkerHost(post: PostFn): WorkerHost {
   ...
   *
   *  `activeAggCols` is supplied by the caller rather than re-derived here:
   *  it is the same set `buildCandidates` must EXCLUDE from the leaf pass,
   *  and deriving it in two places is how the two halves of
   *  `groupAggFiltering` drift apart. */
  function pruneGroupsByAggFilter(
    ids: readonly string[],
    activeAggCols: ReadonlySet<string>,
  ): GroupPassOutput | null {
    if (!state) return null;
    const out = state.groupOutput!;
    if (activeAggCols.size === 0) return null;
  ...
  async function buildVisibleAsync(): Promise<string[]> {
    ...
    if (aggFiltering && !state.groupOutput.bypassed && state.groupOutput.roots.length > 0) {
      const pruned = pruneGroupsByAggFilter(ids, aggFilterCols!);
  ...
  async function visibleAsync(): Promise<string[]> {
    ...
    if (!state.visibleCache) {
      // Memoize-on-miss: a FILL of the cache for the current model, not a
      // change to it, so the generation stays put (see visibleModel.ts).
  ...
  /** Cycle 15 / Task 16 — the CSRM sticky ancestor band for `rowStart`.
   *  Group rows come from the collapse-aware visible order; their display
   *  fields come from `metaLookup`. The walk itself is shared with the
   *  sparse SSRM band — see `collectStickyAncestors`. */
  function computeStickyAncestors(
    ...
    return collectStickyAncestors(
      Math.min(rowStart, visibleOrder.length),
      (i) => {
        const entry = visibleOrder[i]!;
        return entry.kind === 'group' ? entry : undefined;
      },
      (entry, depth) => {
        const meta = metaLookup.get(entry.key);
        // A collapsed (or unknown) group ends the chain.
        if (!meta || !meta.isExpanded) return null;
        return {
          depth,
          key: entry.key,
          colId: meta.colId,
  ...
  async function invalidateAndCount(): Promise<number> {
    if (!state) return 0;
    // Unconditional rebuild — this is the one path every model mutation
    // funnels through, including the group / pivot ones that never nulled
    // the cache themselves, so it is where their generation bump lands.
    const visible = await buildVisibleAsync();
    setVisibleModel(state, visible);
    if (isGroupingActive()) {
